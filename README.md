...
If a task wants to access the resource, it checks the semaphore's value:
if non-zero, the task aquires the semaphore and accesses the resource.
the Task decrements the semaphore value when they acquire it and increment it when they release the resource.
